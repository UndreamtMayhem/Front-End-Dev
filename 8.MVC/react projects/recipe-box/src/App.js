...
class App extends Component {
      ...
      starters: [
        {
          ...
          price: '12.50',
          ingredients: ['white beans', 'herbs', 'onion', 'garlic', 'crostini or bruschetta']
        }, {
          id: 1,
  ...
  // ////////////////////////////// Set Default Recipes VIA  local storage or react
  // set state set local storage NEXT
  setRecipes() {
    if (typeof(Storage) !== "undefined") {
      ...
      if (!localStorage.getItem("_undreamtmayhem_recipe")) {
        ...
        this.setState({recipe: saveToSetState});
  ...
  addRecipe(category, recipe) {
    ...
    if (localStorageAvailablity) {
      ...
      $('#add-recipe .close').click(// use react set state as alternative
      );
  ...
  // Store the item we want to Edit works on starters and gets catergory and id
  ...
  deleteRecipe(id, category) {
    ...
    this.populateLocalStorage(recipe);
    // set Local storage let itemToDelete = this.state.recipe[category]; let index =
    // itemToDelete.findIndex(x => x.id === id); itemToDelete.splice(index, 1);
